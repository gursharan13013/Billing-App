...
import { TransactionType, Language, CompanyProfile, Party, APP_VERSION, AppSettings } from '../types';
import { BillingService as billingService } from '../src/services/SecureBillingService';
...
import { firebaseAuthError, signInWithGoogle } from '../src/services/firebaseService';
...
export const DashboardScreen: React.FC<DashboardScreenProps> = ({ 
  ...
  const menuItems = [
    ...
    { label: t.contra, icon: <ArrowRightLeft size={18} className="text-orange-500 dark:text-orange-400" />, action: onContra, bgColor: 'bg-orange-500/10 dark:bg-orange-500/15', category: 'cash' },
    { label: t.journal, icon: <BookOpen size={18} className="text-slate-500 dark:text-slate-400" />, action: onJournal, bgColor: 'bg-slate-500/10 dark:bg-slate-500/15', category: 'cash' },

    { label: t.chat, icon: <MessageSquare size={18} className="text-purple-500 dark:text-purple-400" />, action: onChat, bgColor: 'bg-purple-500/10 dark:bg-purple-500/15', category: 'misc' },
    { label: t.attendance, icon: <Users size={18} className="text-pink-500 dark:text-pink-400" />, action: onAttendance || (() => {}), bgColor: 'bg-pink-500/10 dark:bg-pink-500/15', category: 'misc' },
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white overflow-hidden relative pb-[max(env(safe-area-inset-bottom),0px)]" onClick={() => isMenuOpen && setIsMenuOpen(false)}>
      ...
      {/* Header Section Redesigned for Premium Slate Look */}
      <div className="p-5 pt-[max(env(safe-area-inset-top),36px)] bg-slate-900 border-b border-slate-800 text-white pb-4 relative shrink-0 shadow-lg">
          <div className="flex items-center justify-between max-w-5xl mx-auto w-full">
              <div className="flex flex-col gap-0.5">
                  <span className="text-[9px] uppercase font-bold tracking-widest text-slate-400 font-mono">Workspace App</span>
                  <h1 className="text-lg font-extrabold tracking-tight text-white line-clamp-1">{profile?.name || 'My Business'}</h1>
              </div>
              ...
              <div className="flex items-center gap-1">
                  ...
                  {isMenuOpen && (
                    <div className="absolute top-16 right-5 w-48 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-50 overflow-hidden divide-y divide-slate-800/60 text-white">
                        ...
                        <button onClick={() => onNavigate('helpLegal')} className="w-full text-left px-4 py-3 hover:bg-slate-800 text-xs font-bold flex items-center gap-2"><Info size={14}/> {t.aboutUs || 'About Us'}</button>
                        ...
                        }} className="w-full text-left px-4 py-3 hover:bg-slate-800 text-xs font-bold flex items-center gap-2"><QrCode size={14}/> {t.qrCode || 'QR Code'}</button>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6 pt-4 relative flex flex-col w-full max-w-6xl mx-auto">
          ...
          {/* Responsive Bento Grid representing modern Billing Software layout */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start w-full">
              
              {/* LEFT COLUMN: Business Terminal Insights & Profile (Capped at 5 of 12 columns on large screens) */}
              <div className="col-span-1 lg:col-span-5 flex flex-col gap-4 bg-white dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800/80 p-5 rounded-2xl shadow-sm transition-all">
                  ...
                  {/* Business Terminal Header Badge */}
                  ...
                  <HeroGraphic qrData={qrData} mobile={profile?.mobile} formattedDate={formattedDate} />
              </div>

              {/* RIGHT COLUMN: Active Operations Desk & Action Shortcuts (7 of 12 columns) */}
