...
export const MasterScreen: React.FC<MasterScreenProps> = ({ 
  ...
  const menuGrid = [
    { label: language === 'hi' ? 'यूनिट जोड़ें' : 'Add Unit', icon: <Ruler size={18} className="text-blue-500" />, bgColor: 'bg-blue-500/10 dark:bg-blue-500/15', action: () => onNavigate && onNavigate('unitList') },
    { label: language === 'hi' ? 'आइटम जोड़ें' : 'Add Item', icon: <Package size={18} className="text-emerald-500" />, bgColor: 'bg-emerald-500/10 dark:bg-emerald-500/15', action: () => onNavigate && onNavigate('itemList') },
    { label: language === 'hi' ? 'ग्रुप जोड़ें' : 'Add Group', icon: <LayoutGrid size={18} className="text-indigo-500" />, bgColor: 'bg-indigo-500/10 dark:bg-indigo-500/15', action: () => onNavigate && onNavigate('accountGroupList') },
    { label: language === 'hi' ? 'ग्राहक जोड़ें' : 'Add Customer', icon: <UserPlus size={18} className="text-indigo-500" />, bgColor: 'bg-indigo-500/10 dark:bg-indigo-500/15', action: () => onNavigate && onNavigate('partyList', { partyMode: 'customer' }) },
    { label: language === 'hi' ? 'HSN जोड़ें' : 'Add HSN', icon: <ScatterChart size={18} className="text-violet-500" />, bgColor: 'bg-violet-500/10 dark:bg-violet-500/15', action: () => onNavigate && onNavigate('hsnList') },
    { label: language === 'hi' ? 'लेजर जोड़ें' : 'Add Ledger', icon: <Book size={18} className="text-violet-500" />, bgColor: 'bg-violet-500/10 dark:bg-violet-500/15', action: () => onNavigate && onNavigate('partyList', { partyMode: 'ledger' }) },
    { label: language === 'hi' ? 'टैक्स केटेगरी' : 'Tax Category', icon: <Scissors size={18} className="text-teal-500" />, bgColor: 'bg-teal-500/10 dark:bg-teal-500/15', action: () => onNavigate && onNavigate('taxList') },
    { label: language === 'hi' ? 'केटेगरी जोड़ें' : 'Add Category', icon: <Briefcase size={18} className="text-teal-500" />, bgColor: 'bg-teal-500/10 dark:bg-teal-500/15', action: () => onNavigate && onNavigate('categoryList') },
    { label: language === 'hi' ? 'ओपनिंग स्टॉक' : 'Opening Stock', icon: <Package size={18} className="text-[#ea580c]" />, bgColor: 'bg-[#ea580c]/10 dark:bg-[#ea580c]/15', action: () => onNavigate && onNavigate('openingStock') },
    { label: language === 'hi' ? 'सेटिंग्स' : 'Settings', icon: <Settings size={18} className="text-slate-500" />, bgColor: 'bg-slate-500/10 dark:bg-slate-500/15', action: onOpenSettings },
  ];
