...
import { Party, InvoiceItem, Item, TransactionType, Language, PaymentRecord, CompanyProfile, Invoice } from '../types';
import { BillingService as billingService, BillingService } from '../src/services/SecureBillingService';
...
import { shareInvoiceWithClient, sharePaymentWithClient } from '../src/services/firebaseService';
...
export const InvoiceScreen: React.FC<InvoiceScreenProps> = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white transition-colors pb-[max(env(safe-area-inset-bottom),0px)]">
      {/* Header */}
      <header className={`${invoiceId ? 'bg-orange-600' : 'bg-blue-700'} text-white p-3 pt-[max(env(safe-area-inset-top),48px)] flex justify-between items-center shadow-md z-20 border-b border-white/10`}>
      ...
      <div className="flex-1 overflow-y-auto pb-44">
        <div className="p-3 space-y-3">
          ...
          {/* Row 1: No & Customer (Z-INDEX 50) */}
          <div className="flex gap-3 relative z-50">
            <div className="w-1/3">
              <label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-bold">{displayType} {labels.no}</label>
              <input 
                type="text" 
                value={saleNo} 
                onChange={(e) => setSaleNo(e.target.value)}
                readOnly={isOnlineImported || !['Purchase', 'Purchase Return'].includes(transactionType)} 
                className={`w-full h-[36px] px-2 border border-slate-300 dark:border-slate-700 rounded text-sm bg-white dark:bg-slate-800 font-bold text-slate-900 dark:text-white focus:outline-none ${isOnlineImported || !['Purchase', 'Purchase Return'].includes(transactionType) ? '' : 'focus:ring-2 focus:ring-blue-500'}`}
              />
            </div>
            <div className="flex-1 relative">
              <label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-bold">{labels.customer}</label>
              {selectedParty ? (
                <div 
                  onClick={() => !isOnlineImported && setSelectedParty(null)} 
                  className={`w-full h-[36px] px-2 border border-blue-300 bg-blue-50 dark:bg-blue-900/40 dark:border-blue-800 rounded text-sm flex justify-between items-center ${!isOnlineImported ? 'cursor-pointer' : ''}`}
                >
                  <div className="min-w-0 flex-1">
                      <div className="font-bold text-blue-800 dark:text-blue-200 truncate">{selectedParty.name}</div>
                  </div>
                  {!isOnlineImported && <span className="text-xs text-blue-600 dark:text-blue-400 font-semibold">{labels.change}</span>}
                </div>
              ) : (
          ...
          {/* Row 2: Item Code & Name (Z-INDEX 40) */}
          {!isOnlineImported && (
          <>
          <div className="flex gap-3 relative z-40">
            <div className="w-1/3">
              <label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-bold">{labels.itemCode}</label>
              <input 
                type="text"
                value={entryItem.code}
                onChange={e => handleCodeChange(e.target.value)}
                onFocus={handleFocus}
                className="w-full h-[36px] px-2 border border-slate-300 dark:border-slate-700 rounded text-sm focus:border-blue-500 outline-none bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-semibold placeholder-slate-400"
              />
            </div>
            <div className="flex-1 relative">
              <label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-bold">{labels.itemName}</label>
              <div className="relative flex items-center h-[36px]">
                <input 
                  ...
                  onFocus={handleFocus}
                  className={`w-full h-[36px] px-2 ${localStorage.getItem('showBarcodeScanner') !== 'false' ? 'pr-8' : 'pr-2'} border border-slate-300 dark:border-slate-700 rounded text-sm focus:border-blue-500 outline-none bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-semibold placeholder-slate-400`}
                  placeholder={labels.searchPlaceholder}
                />
                {localStorage.getItem('showBarcodeScanner') !== 'false' && (
                    <button 
                        onClick={startScanner}
                        className="absolute right-1 text-slate-500 dark:text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 p-1"
                        title="Scan Barcode"
                    >
                      <ScanBarcode size={20} />
                    </button>
              ...
              {/* Autocomplete Dropdown - Theme Aware */}
              {showSuggestions && itemSuggestions.length > 0 && (
                 <div className="absolute top-full left-0 right-0 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl max-h-48 overflow-y-auto rounded-b mt-1 z-50">
                    {itemSuggestions.map(item => (
                      <div 
                        key={item.id}
                        className="p-3 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer text-base border-b last:border-0 border-slate-100 dark:border-slate-700"
                        onClick={() => handleSelectItem(item)}
                      >
                        <div className="font-bold text-slate-900 dark:text-white text-lg">{item.name}</div>
                        <div className="text-sm text-slate-500 dark:text-slate-400 flex justify-between font-medium">
                          <span>Code: {item.id}</span>
                          <span>Rate: {item.saleRate}</span>
                        </div>
                      </div>
                    ))}
                 </div>
          ...
          {/* Row 3: MRP, Rate, Qty, Add */}
          <div className="flex gap-2 items-end relative z-30">
             <div className="flex-1">
                <label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-bold">{labels.mrp}</label>
                <input 
                  type="number" 
                  value={entryItem.mrp}
                  onChange={e => setEntryItem({...entryItem, mrp: parseFloat(e.target.value) || 0})}
                  onFocus={handleFocus}
                  className="w-full h-[36px] px-2 border border-slate-300 dark:border-slate-700 rounded text-sm focus:border-blue-500 outline-none bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-semibold"
                />
             </div>
             <div className="flex-1">
                <label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-bold">{labels.rate}</label>
                <input 
                  type="number" 
             ...
             <div className="w-24">
                <label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-bold">{labels.qty}</label>
                <input 
                  ref={qtyInputRef} // ATTACH REF HERE
                  type="number" 
                  value={entryItem.qty}
                  onChange={e => setEntryItem({...entryItem, qty: parseFloat(e.target.value) || 0})}
                  onFocus={handleFocus}
                  className="w-full h-[36px] px-2 border border-slate-300 dark:border-slate-700 rounded text-sm focus:border-blue-500 outline-none text-center bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-bold ring-1 ring-transparent focus:ring-blue-500"
                />
             </div>
             
             {/* Button Area - Adjusted for mobile */}
             <div className="flex gap-1">
                 {editingItemId ? (
                     <>
                        <button 
                           onClick={cancelEdit}
                           className="bg-red-500 text-white rounded px-2 py-1 h-[36px] shadow active:bg-red-600 hover:bg-red-600 border border-red-500 flex items-center justify-center"
                           title="Cancel Edit"
                        >
                           <X size={18} />
                        </button>
                        <button 
                           ...
                           className="bg-blue-600 text-white font-bold rounded px-3 py-1 h-[36px] text-sm shadow active:bg-blue-700 hover:bg-blue-700 border border-blue-500"
                        >
                           {labels.update}
                        </button>
                     </>
          ...
          {/* Row 4: Tax Type, Tax%, Disc% */}
          <div className="flex gap-2 relative z-20">
             {companyProfile?.isGstRegistered && (
                 <>
                     <div className="w-1/3">
                        <label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-bold">{labels.taxType}</label>
                        <div className="relative h-[36px]">
                          <select 
                            value={entryItem.taxType}
                            onChange={e => setEntryItem({...entryItem, taxType: e.target.value as any})}
                            className="w-full h-[36px] pl-2 pr-6 border border-slate-300 dark:border-slate-700 rounded text-sm appearance-none focus:border-blue-500 outline-none bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-semibold"
                          >
                            <option value="Excluded">{labels.excluded}</option>
                            <option value="Included">{labels.included}</option>
                          </select>
                          <ChevronDown size={14} className="absolute right-2 top-2.5 pointer-events-none text-slate-400" />
                        </div>
                     </div>
                     <div className="flex-1">
                        <label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-bold">{language === 'hi' ? 'परचेस रेट' : 'Purchase Rate'}</label>
                        <input 
                          type="number"
             ...
             <div className="flex-1">
                <div className="flex justify-between items-end mb-1">
                    <label className="block text-sm text-slate-600 dark:text-slate-300 font-bold">{labels.disc}(%)</label>
                    {entryItem.discPercent > 0 && (
                        <span className="text-xs font-bold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30 px-1.5 py-0.5 rounded border border-green-200 dark:border-green-800/50">
                            {language === 'hi' ? 'नेट रेट' : 'Net Rate'}: ₹{(entryItem.rate - (entryItem.rate * (entryItem.discPercent || 0) / 100)).toFixed(2)}
                        </span>
                    )}
                </div>
                <input 
          ...
          <div className="flex justify-between items-center pt-2 border-t border-slate-200 dark:border-slate-800 mt-2">
             <div className="flex gap-3 items-center">
                 <span className="text-sm font-bold text-slate-500 dark:text-slate-400">{labels.stock} : {entryItem.itemRef ? (entryItem.itemRef.openingStock || 0) : '0'}</span>
                 {entryItem.itemRef && <span className="text-sm font-bold text-blue-600 dark:text-blue-400 px-2 py-0.5 bg-blue-50 dark:bg-blue-900/40 rounded border border-blue-100 dark:border-blue-800">{language === 'hi' ? 'सेल रेट' : 'Sale Rate'} : ₹{entryItem.itemRef.saleRate || 0}</span>}
             </div>
             <span className="text-xl font-bold text-slate-800 dark:text-white">{labels.lineTotal} : ₹{lineTotal.toFixed(2)}</span>
          </div>
          </>
          )}
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 shadow-[0_-2px_10px_rgba(0,0,0,0.1)] dark:shadow-[0_-2px_10px_rgba(0,0,0,0.5)] z-30 transition-colors">
        ...
        {!isOnlineImported && (
        <div className="flex p-2 gap-2 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
           <button 
                onClick={() => handleSave(true)}
                className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 sm:py-2.5 rounded-lg font-bold text-sm flex items-center justify-center gap-1.5 shadow-sm transition-colors active:scale-95"
           >
              <Share2 size={18} />
              {labels.saveAndShare}
           </button>
           <button 
                ...
                className="flex-1 bg-blue-700 hover:bg-blue-800 text-white py-3 sm:py-2.5 rounded-lg font-bold text-sm flex items-center justify-center gap-1.5 shadow-sm transition-colors active:scale-95"
           >
              <Save size={18} />
