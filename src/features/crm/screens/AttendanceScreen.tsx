...
export const AttendanceScreen: React.FC<AttendanceScreenProps> = ({ onBack, language, initialDate }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-app)] text-[var(--text-main)] transition-colors pb-[max(env(safe-area-inset-bottom),0px)] relative">
      ...
      {/* Modern Header */}
      <header className="bg-[var(--bg-card)] border-b border-[var(--border-ui)] text-[var(--text-main)] p-4 pt-[max(env(safe-area-inset-top),38px)] flex items-center justify-between shadow-md shrink-0">
        <div className="flex items-center gap-3">
          <button 
            type="button"
            onClick={onBack} 
            className="p-2 -ml-1 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-[var(--text-secondary)] hover:text-[var(--text-main)] transition-colors cursor-pointer"
          >
            <ArrowLeft size={22} />
          </button>
          <div>
            <span className="text-[9px] uppercase font-bold tracking-widest text-[var(--brand-primary)] font-mono">Operations console</span>
        ...
        {activeTab === 'workers' && (
          <button
            ...
            className="px-3.5 py-1.5 text-xs font-black bg-[var(--brand-primary)] hover:bg-[var(--brand-hover)] rounded-xl flex items-center gap-1.5 transition-all shadow-sm shadow-indigo-600/10 cursor-pointer"
          >
      ...
      <div className="bg-[var(--bg-card)] border-b border-[var(--border-ui)] p-1.5 shadow-2xs">
        <div className="max-w-2xl mx-auto w-full flex gap-1">
          <button 
            onClick={() => setActiveTab('daily')}
          className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold flex flex-col items-center gap-1 transition-all relative ${
            activeTab === 'daily' ? 'text-[var(--brand-primary)] font-black' : 'text-[var(--text-secondary)] hover:text-[var(--text-main)]'
          }`}
        >
          ...
          {activeTab === 'daily' && (
            <motion.div 
              ...
              className="absolute inset-0 bg-[var(--brand-light)] rounded-xl border border-[var(--brand-primary)]/10" 
        ...
        <button 
          ...
          className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold flex flex-col items-center gap-1 transition-all relative ${
            activeTab === 'report' ? 'text-[var(--brand-primary)] font-black' : 'text-[var(--text-secondary)] hover:text-[var(--text-main)]'
          }`}
        >
        ...
        <button 
          ...
          className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold flex flex-col items-center gap-1 transition-all relative ${
            activeTab === 'workers' ? 'text-[var(--brand-primary)] font-black' : 'text-[var(--text-secondary)] hover:text-[var(--text-main)]'
          }`}
        >
      ...
      <div className="flex-1 overflow-y-auto">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'daily' && (
            ...
            >
              {/* Datepicker Bar */}
              <div className="bg-[var(--bg-card)] p-3.5 rounded-2xl shadow-sm border border-[var(--border-ui)] flex items-center justify-between">
                <button 
                  type="button"
                  onClick={() => changeDateByAmount(-1)} 
                  className="p-2 bg-[var(--bg-app)] hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-all active:scale-95 cursor-pointer"
                >
                  <ChevronLeft size={18} />
                </button>
                ...
                <div className="flex flex-col items-center gap-0.5">
                  ...
                  <div className="flex items-center gap-1.5 focus-within:ring-2 focus-within:ring-indigo-500 rounded-lg px-2">
                    ...
                    <input 
                      ...
                      className="font-black text-sm bg-transparent outline-none text-[var(--text-main)] cursor-pointer [color-scheme:light] dark:[color-scheme:dark]"
                    />
                  </div>
                </div>
              ...
              {/* Stats Panel */}
              {workers.length > 0 && (
                <div className="bg-[var(--bg-card)] border border-[var(--border-ui)] rounded-2xl p-4 shadow-xs">
                  <div className="flex items-center justify-between mb-3 border-b border-[var(--border-ui)]/50 pb-2">
                    <h4 className="text-[11px] uppercase tracking-wider font-black text-slate-700 dark:text-slate-300">{t.summary}</h4>
                    <span className="bg-[var(--brand-light)] text-[var(--brand-primary)] px-2 py-0.5 rounded text-[10px] font-black tracking-semibold">
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center select-none">
                    ...
                    <div className="bg-[var(--bg-app)] border border-[var(--border-ui)]/40 p-2 rounded-xl">
              ...
              {/* Search worker */}
              {workers.length > 0 && (
                <div className="relative flex items-center bg-[var(--bg-card)] border border-[var(--border-ui)] px-3.5 py-1.5 rounded-2xl shadow-2xs">
                  ...
                  <input 
                    ...
                    className="w-full text-xs bg-transparent border-none outline-none text-[var(--text-main)]"
                  />
                  {searchQuery && (
              ...
              <div className="space-y-3">
                {workers.length === 0 ? (
                  <div className="bg-white dark:bg-slate-900 border border-slate-150 border-dashed dark:border-slate-800/80 rounded-2xl py-12 px-6 text-center shadow-2xs animate-pulse">
                    ...
                    <h3 className="text-sm font-black text-[var(--text-main)]">{t.noWorkers}</h3>
                  ...
                  filteredWorkers.map(w => {
                    ...
                    return (
                      <div 
                        key={w.id} 
                        className="bg-[var(--bg-card)] p-4 rounded-2xl shadow-sm border border-[var(--border-ui)]/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:shadow-md"
                      >
                        {/* Profile Info Details */}
                        <div className="flex items-center gap-3">
                          <div className="w-11 h-11 bg-gradient-to-br from-[var(--brand-light)] to-[var(--bg-app)] border border-indigo-200/20 rounded-2xl flex items-center justify-center font-black text-base text-[var(--brand-primary)] shrink-0">
                            {letter}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <h3 className="font-extrabold text-sm text-[var(--text-main)] leading-snug">{w.name}</h3>
                              {status ? (
                          ...
                          <button 
                            ...
                            className={`flex-1 sm:flex-initial px-4 py-2 rounded-xl text-xs font-black tracking-wider transition-all border cursor-pointer ${
                              status === 'Present' 
                                ...
                                : 'bg-[var(--bg-app)] text-[var(--text-secondary)] border-[var(--border-ui)]/50 hover:bg-emerald-50 dark:hover:bg-emerald-950/20'
                            }`}
                          >
                            P
                          </button>
                          ...
                          <button 
                            ...
                            className={`flex-1 sm:flex-initial px-4 py-2 rounded-xl text-xs font-black tracking-wider transition-all border cursor-pointer ${
                              status === 'Half-Day' 
                                ...
                                : 'bg-[var(--bg-app)] text-[var(--text-secondary)] border-[var(--border-ui)]/50 hover:bg-amber-50 dark:hover:bg-amber-950/20'
                            }`}
                          >
                            HD
                          </button>
                          ...
                          <button 
                            ...
                            className={`flex-1 sm:flex-initial px-4 py-2 rounded-xl text-xs font-black tracking-wider transition-all border cursor-pointer ${
                              status === 'Absent' 
                                ...
                                : 'bg-[var(--bg-app)] text-[var(--text-secondary)] border-[var(--border-ui)]/50 hover:bg-rose-50 dark:hover:bg-rose-950/20'
                            }`}
                          >
                            A
                          </button>
          ...
          {activeTab === 'report' && (
            ...
            >
              ...
              <div className="bg-[var(--bg-card)] p-4 rounded-2xl shadow-sm border border-[var(--border-ui)] flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
                <div className="flex items-center gap-3">
              ...
              <div className="space-y-3">
                {workers.length === 0 ? (
                  <div className="bg-[var(--bg-card)] border border-[var(--border-ui)] border-dashed rounded-2xl py-12 px-6 text-center animate-pulse">
                    <Users size={32} className="text-indigo-400/60 mx-auto mb-3" />
                    <h3 className="text-sm font-black text-[var(--text-main)]">{t.noWorkers}</h3>
                    <p className="text-xs text-slate-500 mt-1">{t.addOne}</p>
                  </div>
                ) : filteredReportData.length === 0 ? (
                  <div className="bg-[var(--bg-card)] border border-[var(--border-ui)] rounded-2xl p-8 text-center text-slate-400">
                  ...
                  filteredReportData.map(r => (
                    <div 
                      key={r.id} 
                      className="bg-[var(--bg-card)] p-4 rounded-2xl shadow-sm border border-[var(--border-ui)]/50 flex flex-col gap-4.5"
                    >
                      ...
                      <div className="flex justify-between items-start gap-4 border-b border-slate-50 dark:border-slate-800/80 pb-3">
                        <div>
                          <h3 className="font-black text-sm text-[var(--text-main)] leading-snug">{r.name}</h3>
                ...
                {workers.length === 0 ? (
                  ...
                  workers.map(w => {
                    ...
                    return (
                      <div 
                        key={w.id} 
                        className="bg-[var(--bg-card)] p-4 rounded-2xl shadow-sm border border-[var(--border-ui)]/50 flex items-center justify-between transition-colors duration-150"
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-indigo-50 to-indigo-100/50 dark:from-slate-800 dark:to-slate-850 rounded-2xl flex items-center justify-center font-black text-[var(--brand-primary)] shrink-0 text-sm">
                            {letter}
                          </div>
                          <div>
                            <h3 className="font-extrabold text-sm text-[var(--text-main)] leading-snug">{w.name}</h3>
              ...
              <form onSubmit={handleSaveWorker} className="p-5 space-y-4">
                ...
                <div className="flex gap-2.5 pt-2">
                  <button 
                    ...
                    className="flex-1 py-3 text-xs font-black border border-slate-150 dark:border-slate-800 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-100 dark:hover:bg-slate-800 text-[var(--text-secondary)] hover:text-[var(--text-main)] transition-colors cursor-pointer"
                  >
                    {t.cancel}
                  </button>
                  <button 
                    type="submit" 
                    className="flex-1 py-3 text-xs font-black bg-[var(--brand-primary)] hover:bg-[var(--brand-hover)] text-white rounded-xl shadow-md cursor-pointer"
                  >
                    {t.saveWorker}
                  </button>
                </div>
