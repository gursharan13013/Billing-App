...
import { TransactionType, Invoice, PaymentRecord, Party } from '../core/types/';
...
export const BusinessReportScreen: React.FC<BusinessReportScreenProps> = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 transition-colors pb-[max(env(safe-area-inset-bottom),0px)]">
      {/* Header */}
      <header className="bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 text-white p-4 px-4 pt-[max(env(safe-area-inset-top),48px)] pb-2 flex items-center justify-between shadow-md shrink-0 border-b border-slate-800/50 dark:border-slate-900 relative">
        <div className="flex items-center gap-2">
            ...
            <div>
                ...
                <p className="text-sm text-slate-400 leading-tight">Detailed View</p>
            </div>
        </div>
            ...
            {selectedIds.length > 0 ? (
                ...
                <>
                    <button 
                        ...
                        className="bg-slate-800 p-1.5 rounded hover:bg-slate-700 transition-colors border border-slate-700/50"
      ...
      {/* Tabs */}
      <div className="bg-slate-950 pb-2.5 px-3 overflow-x-auto no-scrollbar border-b border-slate-900">
              ...
              {tabs.map(tab => (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all border ${
                        ...
                        : 'bg-slate-900/80 text-slate-400 border-slate-800/60 hover:bg-slate-800'
                    }`}
                  >
                      {tab}
                  </button>
