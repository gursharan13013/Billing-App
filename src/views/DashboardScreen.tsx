...
export const DashboardScreen: React.FC<DashboardScreenProps> = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white overflow-hidden relative pb-[max(env(safe-area-inset-bottom),0px)]" onClick={() => isMenuOpen && setIsMenuOpen(false)}>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6 pt-4 relative flex flex-col w-full max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start w-full">
              ...
              <div className="col-span-1 lg:col-span-7 flex flex-col gap-4">
                  ...
                  {/* Custom container for grid menu */}
                  <div>
      ...
      {/* Viewport Floating Custom Buttons Stacked at Bottom Right (Draggable) */}
      {localStorage.getItem('showBarcodeScanner') !== 'false' && (
          <DraggableFAB
            id="scanner_btn"
            defaultPosition={(w, h) => {
              // Stacked above the Smart Assistant
              return { x: w - 76, y: h - 228 };
            }}
            persistPosition={true}
            onClick={startScanner}
          >
              <div className="w-14 h-14 bg-rose-600 border-4 border-white dark:border-slate-850 rounded-full flex items-center justify-center shadow-lg active:scale-90 transition-transform pointer-events-auto">
                  <ScanLine size={24} className="text-white" />
              </div>
          </DraggableFAB>
      )}

      {/* AI Assistant Component handles its button and layout relative to Main Viewport wrapper */}
      {localStorage.getItem('showSmartAssistant') !== 'false' && (
          <AIAssistant 
            onNavigate={onNavigate} 
            defaultPosition={(w, h) => {
              // Placed elegantly above the bottom relative layout navigation
              return { x: w - 76, y: h - 156 };
            }}
            persistPosition={true}
            language={language}
          />
